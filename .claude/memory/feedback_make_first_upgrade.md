---
name: Template subtree 升級先用 make
description: 升級 template subtree 一律 make 優先，./template/upgrade.sh 留作 fallback
type: feedback
originSessionId: 33bd5c5e-e564-48bf-b038-bcaab5b8b2f6
---
升級 template subtree 一律先用 make，只在 make 不可用 / target 出問題時才退回 `./template/upgrade.sh`：

- 升到最新：`make -f Makefile.ci upgrade`
- 指定版本：`make -f Makefile.ci upgrade VERSION=vX.Y.Z`
- 檢查新版：`make -f Makefile.ci upgrade-check`
- Fallback：`./template/upgrade.sh [vX.Y.Z]`
...
範例指令、CLAUDE.md「template subtree 更新流程」、template README「Updating」
章節都要以 make 為主。
