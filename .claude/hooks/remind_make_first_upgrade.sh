...
# `template/upgrade.sh` (or `./template/upgrade.sh` / absolute path variant)
...
# 規定升級一律 make 優先,`./template/upgrade.sh` 留作 fallback,只在 make
# 不可用或 target 出問題時才用。這層提醒避免 agent 記憶體裡只剩
# upgrade.sh、跳過 make wrapper 的 init.sh resync / main.yaml @tag rewrite
# 自動處理。
#
# Detection:
#   1. Match `template/upgrade.sh` invocations (`./template/upgrade.sh`,
#      `template/upgrade.sh`, absolute path).
#   2. Resolve work dir.
...
main() {
  ...
  # Match upgrade.sh invocations. Allow `./template/upgrade.sh`,
  # `template/upgrade.sh`, or absolute `/...../template/upgrade.sh`.
  [[ "${cmd}" =~ (^|[[:space:]\;\&\|])(\./)?(.*/)?template/upgrade\.sh([[:space:]]|$) ]] || return 0
  ...
  # Try to extract version arg if present (e.g. `./template/upgrade.sh v0.18.2`).
  version_arg=""
  if [[ "${cmd}" =~ template/upgrade\.sh[[:space:]]+(v[0-9][0-9.]*(-[A-Za-z0-9.]+)?) ]]; then
    version_arg=" VERSION=${BASH_REMATCH[1]}"
  fi

  msg="$(printf 'template upgrade 提醒：偵測到直接跑 ./template/upgrade.sh,優先改用 make wrapper:\n  make -f Makefile.ci upgrade%s\nmake target 內部會呼叫同一支 upgrade.sh,但會幫你跑 init.sh resync + main.yaml @tag sed,降低漏步的機率。只在 make 不可用或 target 出問題時才 fallback 直接跑 .sh。' \
    "${version_arg}")"
