...
- `check_tag_version_consistency.sh` — `git tag v*` / `git push v*` 前
  ...
  人工約束）
- `enforce_make_first_upgrade.sh` — 直接跑 `./.base/upgrade.sh` 時
  **BLOCK** 並要求改走 `make -f Makefile.ci upgrade`(make wrapper 內部呼
  叫同一支 .sh,但會幫忙跑 init.sh resync + main.yaml @tag sed)。要 lift
  gate 跑 `/tmp` checkpoint protocol(ADR-00000002):touch deny 訊息列的
  `<ack>` 檔再重發同一條指令即可
