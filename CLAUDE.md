...
3. **Docker 驗證**：跑一次 `make -f Makefile.ci test`（template）或
...
└── .claude/                  # Claude Code 設定
    ...
    │   ├── batch-template-v0.21.1-stage-rename.sh # 一次性 v0.21.0->v0.21.1 stage rename + runtime-test 12 repo fanout（template #243 / #250）
...
make -f Makefile.ci upgrade                   # 升級 template subtree 到最新 tag
...
- `make -f Makefile.ci test` / `lint` — template 自身的 unit/integration 測試
...
使用 Semantic Versioning + Release Candidate：

| 格式 | 用途 | 範例 |
|------|------|------|
| `MAJOR.MINOR.0` | 功能版本（新功能、行為變更） | `v1.3.0` |
| `MAJOR.MINOR.PATCH` | Bug fix / 文件修正 | `v1.3.1` |
| `MAJOR.MINOR.0-rcN` | Release Candidate（正式發布前驗證） | `v1.3.0-rc1` |

MAJOR 保留給破壞性變更（如 .env 格式改變）。
...
# 4. RC 失敗 → 修復後打 rc2
git tag -a v1.3.0-rc2 -m "v1.3.0-rc2: fix ..."
...
### template subtree 更新流程

```bash
# 1. 在 template 本體修改 + commit + push + tag (use latest version)
